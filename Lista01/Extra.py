...
#Constantes
sigma = 25
Foto_borrada = loadtxt('blur.txt', float)
...
for i in range(y_dim):
    for j in range(x_dim):
        Ponto_de_Propagacao_array[i, j] = Ponto_de_Propagacao( (j + y_dim / 2) % y_dim - y_dim / 2, \
...
for i in range(x_dim // 2 + 1):
    for j in range(y_dim):
        if abs(Ponto_de_Propagacao_fourier[j, i]) < epsilon:
            ...
            Nao_borrada_fourier[j, i] = Foto_borrada_fourier[j, i] / (Ponto_de_Propagacao_fourier[j, i])
